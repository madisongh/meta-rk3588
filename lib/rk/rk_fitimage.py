#
# Adapted from oe/fitimage.py from OE-Core,
# with modifications specifc to Rockchip downstream
# BSP tooling
#
...
class ItsNodeRootKernel(ItsNode):
    ...
    def __init__(self, description, address_cells, host_prefix, arch, conf_prefix,
        ...
        self._dtb_to_update = dtb_to_update
    ...
    def run_mkimage_assemble(self, itsfile, fitfile):
        cmd = [self._mkimage] + self._mkimage_extra_opts
        ...
        if self._dtb_to_update:
            cmd += [ '-K', self._dtb_to_update ]
