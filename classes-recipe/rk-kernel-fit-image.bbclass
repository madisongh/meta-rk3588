# Adapted from OE-Core kernel-fit-image.bbclass,
# stripped down to support only those features that
# are in the Rockchip downstream BSP/SDK.

inherit kernel-arch kernel-artifact-names rk-uboot-sign uboot-config deploy
...
DEPENDS += "\
    ...
    ${@'virtual/bootloader' if oe.types.boolean(d.getVar('FIT_KERNEL_SIGN_ENABLE')) else ''} \
...
python do_compile() {
    ...
    uboot_dtb_binary = d.getVar('UBOOT_DTB_BINARY')
    # Collect all the its nodes before the its file is generated and mkimage gets executed
    root_node = rk.rk_fitimage.ItsNodeRootKernel(
    ...
    if uboot_dtb_binary:
        # For DTB signing, we need the DTB locally, and
        # that is staged in the recipe sysroot. Use a glob
        # to get the symlink(s) and the actual file.
        builddir = d.getVar('B')
        for dtbf in glob.glob(os.path.join(d.getVar('STAGING_DATADIR'), 'u-boot*.dtb')):
            dtbf_basename = os.path.basename(dtbf)
            shutil.copy(dtbf, os.path.join(builddir, dtbf_basename), follow_symlinks=False)
    ...
    if external_kernel_devicetree:
        ...
        for dtb_name in sorted(os.listdir(external_kernel_devicetree)):
            if dtb_name.endswith('.dtb') or dtb_name.endswith('.dtbo'):
                ...
                symlink_target = rk.rk_fitimage.symlink_points_below(dtb_name, external_kernel_devicetree)
                if symlink_target:
...
do_deploy() {
    ...
    if [ "${FIT_KERNEL_SIGN_ENABLE}" = "1" -a -n "${UBOOT_DTB_BINARY}" ]; then
        # UBOOT_DTB_IMAGE is a realfile, but we can't use
        # ${UBOOT_DTB_IMAGE} since it contains ${PV} which is aimed
        # for u-boot, but we are in a different recipe now.
        install -m 0644 ${B}/u-boot-${MACHINE}*.dtb "$deploy_dir/"
    fi
}
